...
combined_data_16s_dec <- read_csv("data/formatted_data/combined_data_16s_dec.csv")
variable_sets_16s_dec <- read_rds("data/variable_sets_16s_dec.rds")
...
# set up data
y_16s_dec <- as.numeric(combined_data_16s_dec$diarrhea)
x_16s_dec <- select(combined_data_16s_dec, -diarrhea, -sample_id,
                -c(alternate_metagenome_id, 
                   ...
                   geometric_mean)) %>% 
  mutate(across(starts_with("dec"), ~.x * 1))  %>% 
  mutate(across(1:3, ~(.x - mean(.x))/sd(.x)))



# --------------Running VIMP + Superlearner ------------------
run_vimp_holdout_set <- function(indx, y_16s_dec, x_16s_dec, ...) {
  # set up learners
  ...
  set.seed(522305)
  vimp_output <- vimp_auc(y_16s_dec, x_16s_dec, SL.library = c("SL.mean", my_libraries), 
                          V = 10, indx = indx, cvControl = sl_cvcontrol, ...)
  vimp_output
}

slurm_map(variable_sets_16s_dec, run_vimp_holdout_set, 
          jobname = "run_vimp", nodes = 70, cpus_per_node = 1, submit = FALSE, 
          libPaths = ("/home/users/graham18/Desktop/R_lib"),
          y_16s_dec = y_16s_dec, x_16s_dec = x_16s_dec,
          method = "method.AUC")
