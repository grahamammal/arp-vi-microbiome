...
# set up data
y_metagenome <- as.numeric(combined_data_metagenome$diarrhea)
...
run_vimp_holdout_set <- function(indx, y_metagenome, x_metagenome, ...) {
  ...
  lasso_screens <- map2(c("SL.ranger", "SL.caretGLMB", "SL.gam", "SL.glm", glmnet_learners$names), "screen.glmnet", c)
  random_forest_screens <- map2(c("SL.ranger", "SL.caretGLMB", "SL.gam", "SL.glm", glmnet_learners$names), "screen.randomForest", c)
...
# 
# start_time <- Sys.time()
# run_vimp_holdout_set(variable_sets_metagenome$Bacteroidetes, y_metagenome, x_metagenome)
# end_time <- Sys.time()
# 
# end_time - start_time